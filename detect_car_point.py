...
train_path = '/cyl_data/car_plate_train/'
test_path = '/cyl_data/car_plate_test/'
imgsize = 178
train_samples = len(common.get_files(train_path))
test_samples = len(common.get_files(test_path))
batch_size = 32
...
class Model(object):
    ...
    def train(self, model, epoch_num):
        ...
        model.fit_generator(data_label(train_path), callbacks=[check_point, early_stop, change_lr],
                            samples_per_epoch=int(train_samples // batch_size), epochs=epoch_num,
                            validation_steps=int(test_samples // batch_size), validation_data=data_label(test_samples))
